...
export function OnboardingPage() {
  ...
  return <div className="flex min-h-dvh flex-col bg-gray-950 p-6 pb-[calc(24px+env(safe-area-inset-bottom))] pt-[calc(28px+env(safe-area-inset-top))] text-white">
    ...
    <div className="flex min-w-0 flex-1 flex-col justify-center py-10">
      <div className="mb-8 grid size-20 place-items-center rounded-[26px] bg-accent text-accent-content"><IntroIcon size={36} /></div>
    ...
    <Button variant="accent" className="flex w-full items-center justify-center gap-2" onClick={() => step < 2 ? setStep(step + 1) : complete()}>{step < 2 ? 'ПРОДОЛЖИТЬ' : 'НАЧАТЬ ДЕНЬ 1'}<ArrowRight size={19} /></Button>
  </div>
}
