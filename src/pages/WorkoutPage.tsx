...
export function WorkoutPage() {
  ...
  return <div className="flex h-screen h-dvh flex-col overflow-hidden bg-surface pb-safe pt-safe">
    <header className="shrink-0 px-5 pt-4"><div className="flex items-center justify-between"><button onClick={() => setExitOpen(true)} className="grid size-12 place-items-center rounded-full bg-card" aria-label="Выйти из тренировки"><X /></button><div className="text-center"><p className="text-xs font-extrabold uppercase tracking-[.12em] text-muted">День {dayNumber}</p><p className="font-black">{positionLabel}</p></div><span className="size-12" /></div><ProgressBar value={progress} className="mt-4" /></header>
    <main className="workout-scroll flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-5 pb-5">
      <AnimatePresence mode="wait">
        ...
        {session.state === 'countdown' && <motion.section key="countdown" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1 }} className="grid min-h-full flex-1 place-items-center text-center"><div><p className="text-sm font-extrabold uppercase tracking-[.18em] text-muted">Приготовься</p><motion.div key={countdown} initial={{ scale: .55, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-4 text-[clamp(6rem,18dvh,9rem)] font-black leading-none tracking-[-.09em]">{countdown || 'СТАРТ'}</motion.div><p className="mt-5 text-xl font-black">{exercise.name}</p></div></motion.section>}

        {(session.state === 'exercise-running' || session.state === 'exercise-paused') && <motion.section key="running" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex min-h-full flex-1 flex-col pt-[clamp(1.5rem,4dvh,2rem)] text-center"><p className="text-sm font-extrabold uppercase tracking-[.15em] text-muted">{session.state === 'exercise-paused' ? 'Пауза' : 'Продолжай'}</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">{exercise.name}</h1><div className="flex flex-1 items-start justify-center pt-[clamp(2rem,10dvh,7rem)]"><div><p data-testid="workout-timer" className="font-mono text-[clamp(4rem,11dvh,5.4rem)] font-black leading-none tracking-[-.08em] tabular-nums">{workoutItem.duration ? formatTime(timedRemaining) : formatTime(elapsedSeconds)}</p><p className="mt-5 text-lg font-bold text-muted">{workoutItem.reps ? `${workoutItem.reps} повторений` : 'Осталось времени'}</p></div></div><div className="flex items-center gap-3"><button onClick={session.state === 'exercise-paused' ? resume : pause} className="grid size-14 shrink-0 place-items-center rounded-2xl border border-line bg-card" aria-label={session.state === 'exercise-paused' ? 'Продолжить' : 'Пауза'}>{session.state === 'exercise-paused' ? <CirclePlay /> : <CirclePause />}</button><Button onClick={() => workoutItem.duration ? setFinishOpen(true) : advanceAfterExercise()} className="flex flex-1 items-center justify-center gap-2">{workoutItem.duration ? <><Flag size={18} />ЗАВЕРШИТЬ РАНЬШЕ</> : <><Check size={20} />ГОТОВО</>}</Button></div></motion.section>}

        {session.state === 'rest' && <motion.section key="rest" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="flex min-h-full flex-1 flex-col pt-[clamp(1.5rem,5dvh,2.5rem)] text-center"><span className="mx-auto grid size-14 shrink-0 place-items-center rounded-full bg-accent text-gray-950"><Check size={26} strokeWidth={3} /></span><h1 className="mt-4 text-4xl font-black tracking-[-.05em]">Отлично!</h1><div className="grid flex-1 place-items-center py-4"><div><p className="text-sm font-extrabold uppercase tracking-[.15em] text-muted">Отдых</p><p className="mt-2 font-mono text-[clamp(4rem,10dvh,5.4rem)] font-black leading-none tracking-[-.08em] tabular-nums">{formatTime(restRemaining)}</p><div className="mx-auto mt-[clamp(1rem,3dvh,1.75rem)] rounded-2xl bg-card px-5 py-3 text-left"><p className="text-xs font-bold uppercase text-muted">Дальше</p><p className="font-black">{exercise.name} · {workoutItem.reps ? `${workoutItem.reps} повт.` : `${workoutItem.duration} сек.`}</p></div></div></div><Button variant="secondary" onClick={() => updateActive({ state: settings.autoNext ? (settings.countdown ? 'countdown' : 'exercise-running') : 'next-exercise', restStartedAt: undefined, exerciseStartedAt: settings.autoNext && !settings.countdown ? Date.now() : undefined })} className="w-full shrink-0">ПРОПУСТИТЬ ОТДЫХ</Button></motion.section>}
