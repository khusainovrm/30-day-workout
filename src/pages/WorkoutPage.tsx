...
export function WorkoutPage() {
  ...
  return <div className="flex min-h-dvh flex-col bg-surface pb-safe pt-safe">
    <header className="px-5 pt-4"><div className="flex items-center justify-between"><button onClick={() => setExitOpen(true)} className="grid size-12 place-items-center rounded-full bg-card" aria-label="Exit workout"><X /></button><div className="text-center"><p className="text-xs font-extrabold uppercase tracking-[.12em] text-muted">Day {dayNumber}</p><p className="font-black">Exercise {session.exerciseIndex + 1} / {day.exercises.length}</p></div><span className="size-12" /></div><ProgressBar value={progress} className="mt-4" /></header>
    <main className="flex flex-1 flex-col px-5 pb-5">
      <AnimatePresence mode="wait">
        {session.state === 'exercise-preview' && <motion.section key="preview" initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }} className="flex flex-1 flex-col pt-6">
          <div className="relative overflow-hidden rounded-[28px] bg-[#e9efcf]"><motion.img key={exercise.id} src={exercise.images[0]} alt={`${exercise.name} technique`} className="aspect-[4/2.7] w-full object-cover" initial={{ opacity: .35 }} animate={{ opacity: 1 }} /></div>
          <div className="mt-5 flex items-start justify-between gap-3"><div><p className="text-sm font-extrabold uppercase tracking-[.12em] text-muted">Up next</p><h1 className="mt-1 text-4xl font-black tracking-[-.05em]">{exercise.name}</h1></div><span className="shrink-0 rounded-2xl bg-card px-4 py-3 text-lg font-black">{workoutItem.reps ? `${workoutItem.reps} reps` : `${workoutItem.duration} sec`}</span></div>
          {!compactTutorial ? <div className="mt-6 rounded-2xl bg-card p-4"><h2 className="flex items-center gap-2 font-black"><Lightbulb size={18} />How to</h2><ol className="mt-3 grid gap-2 text-sm text-muted">{exercise.instructions.map((instruction, index) => <li key={instruction} className="flex gap-3"><b className="text-ink">{index + 1}.</b>{instruction}</li>)}</ol>{exercise.tips?.[0] && <p className="mt-4 border-t border-line pt-3 text-sm"><strong>Tip:</strong> <span className="text-muted">{exercise.tips[0]}</span></p>}</div> : <p className="mt-5 text-sm text-muted">You’ve seen this guide before. Keep your form controlled and breathe steadily.</p>}
          <div className="mt-auto pt-5"><Button onClick={start} className="flex w-full items-center justify-center gap-2"><Play size={19} fill="currentColor" />START</Button></div>
        </motion.section>}

        {session.state === 'countdown' && <motion.section key="countdown" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1 }} className="grid flex-1 place-items-center text-center"><div><p className="text-sm font-extrabold uppercase tracking-[.18em] text-muted">Get ready</p><motion.div key={countdown} initial={{ scale: .55, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-4 text-[9rem] font-black leading-none tracking-[-.09em]">{countdown || 'GO'}</motion.div><p className="mt-5 text-xl font-black">{exercise.name}</p></div></motion.section>}

        {(session.state === 'exercise-running' || session.state === 'exercise-paused') && <motion.section key="running" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-1 flex-col pt-8 text-center"><p className="text-sm font-extrabold uppercase tracking-[.15em] text-muted">{session.state === 'exercise-paused' ? 'Paused' : 'Keep going'}</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">{exercise.name}</h1><div className="grid flex-1 place-items-center"><div><p className="font-mono text-[5.4rem] font-black leading-none tracking-[-.08em] tabular-nums">{workoutItem.duration ? formatTime(timedRemaining) : formatTime(elapsedSeconds)}</p><p className="mt-5 text-lg font-bold text-muted">{workoutItem.reps ? `${workoutItem.reps} repetitions` : 'Time remaining'}</p></div></div><div className="flex items-center gap-3"><button onClick={session.state === 'exercise-paused' ? resume : pause} className="grid size-14 shrink-0 place-items-center rounded-2xl border border-line bg-card" aria-label={session.state === 'exercise-paused' ? 'Resume' : 'Pause'}>{session.state === 'exercise-paused' ? <CirclePlay /> : <CirclePause />}</button><Button onClick={() => workoutItem.duration ? setFinishOpen(true) : advanceAfterExercise()} className="flex flex-1 items-center justify-center gap-2">{workoutItem.duration ? <><Flag size={18} />FINISH EARLY</> : <><Check size={20} />DONE</>}</Button></div></motion.section>}

        {session.state === 'rest' && <motion.section key="rest" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="flex flex-1 flex-col pt-10 text-center"><span className="mx-auto grid size-14 place-items-center rounded-full bg-accent text-gray-950"><Check size={26} strokeWidth={3} /></span><h1 className="mt-4 text-4xl font-black tracking-[-.05em]">Great job!</h1><div className="grid flex-1 place-items-center"><div><p className="text-sm font-extrabold uppercase tracking-[.15em] text-muted">Rest</p><p className="mt-2 font-mono text-[5.4rem] font-black leading-none tracking-[-.08em] tabular-nums">{formatTime(restRemaining)}</p><div className="mx-auto mt-7 rounded-2xl bg-card px-5 py-3 text-left"><p className="text-xs font-bold uppercase text-muted">Next</p><p className="font-black">{exercise.name} · {workoutItem.reps ? `${workoutItem.reps} reps` : `${workoutItem.duration} sec`}</p></div></div></div><Button variant="secondary" onClick={() => updateActive({ state: settings.autoNext ? (settings.countdown ? 'countdown' : 'exercise-running') : 'next-exercise', restStartedAt: undefined, exerciseStartedAt: settings.autoNext && !settings.countdown ? Date.now() : undefined })} className="w-full">SKIP REST</Button></motion.section>}

        {session.state === 'next-exercise' && <motion.section key="next" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-1 flex-col pt-10 text-center"><p className="text-sm font-extrabold uppercase tracking-[.15em] text-muted">Ready when you are</p><h1 className="mt-2 text-4xl font-black tracking-[-.05em]">{exercise.name}</h1><div className="grid flex-1 place-items-center"><img src={exercise.images[0]} alt="" className="w-full rounded-[28px]" /></div><Button onClick={startNext} className="flex w-full items-center justify-center gap-2">START NEXT<ChevronRight size={20} /></Button></motion.section>}
      </AnimatePresence>
    </main>
    <Sheet open={exitOpen} title="Workout in progress" onClose={() => setExitOpen(false)}><p className="text-muted">Your completed exercises are saved. This day will not count as complete.</p><div className="mt-6 grid gap-3"><Button onClick={() => setExitOpen(false)}>KEEP WORKOUT</Button><Button variant="secondary" onClick={exit}>EXIT WORKOUT</Button></div></Sheet>
    <Sheet open={finishOpen} title="Finish this exercise early?" onClose={() => setFinishOpen(false)}><p className="text-muted">It will count as completed and you’ll move to your rest.</p><div className="mt-6 grid gap-3"><Button onClick={() => { setFinishOpen(false); advanceAfterExercise() }}>FINISH EARLY</Button><Button variant="secondary" onClick={() => setFinishOpen(false)}>KEEP GOING</Button></div></Sheet>
  </div>
}
...
function Completion({ programId, day, duration, onDone }: { programId: string; day: number; duration: number; onDone: () => void }) {
  ...
  return <div className="relative flex min-h-dvh flex-col overflow-hidden bg-ink p-6 pb-[calc(24px+env(safe-area-inset-bottom))] pt-[calc(32px+env(safe-area-inset-top))] text-white">
    ...
    <div className="relative flex flex-1 flex-col items-center justify-center text-center"><motion.div initial={{ scale: .6, rotate: -10 }} animate={{ scale: 1, rotate: 0 }} className="grid size-24 place-items-center rounded-[32px] bg-accent text-gray-950"><Trophy size={44} /></motion.div><p className="mt-8 text-sm font-black uppercase tracking-[.18em] text-accent">Challenge progress</p><h1 className="mt-2 text-5xl font-black tracking-[-.06em]">Day {day}<br />Complete</h1><div className="mt-10 grid w-full grid-cols-3 divide-x divide-white/15 rounded-[24px] border border-white/10 bg-white/5 py-5"><div><b className="block text-xl">{formatTime(duration)}</b><span className="text-xs text-white/50">Workout</span></div><div><b className="block text-xl">{item.exercises.length}</b><span className="text-xs text-white/50">Exercises</span></div><div><b className="block text-xl">+1</b><span className="text-xs text-white/50">Day done</span></div></div></div>
    <div className="relative grid gap-3"><Button className="bg-accent text-gray-950" onClick={onDone}>DONE</Button><Button variant="ghost" className="text-white" onClick={() => navigate('/progress')}>VIEW PROGRESS</Button></div>
  </div>
}
