...
export function CategoryPage() {
  ...
  return <div className="px-5 pt-3">
    <Link to="/" className="grid size-12 place-items-center rounded-full bg-card" aria-label="Назад на главную"><ArrowLeft /></Link>
    <p className="mt-8 text-sm font-extrabold uppercase tracking-[.16em] text-muted">{categoryMeta[typedCategory].eyebrow}</p>
    <h1 className="mt-1 text-4xl font-black tracking-[-.05em]">Выбери уровень</h1>
    <p className="mt-3 max-w-sm leading-6 text-muted">Нагрузка должна быть ощутимой, но посильной. План можно сменить в любое время.</p>
    <div className="mt-7 grid gap-5">{(Object.keys(difficultyMeta) as Difficulty[]).map((difficulty, difficultyIndex) => {
      ...
      return <section key={difficulty} className="rounded-[26px] border border-line bg-card p-5">
        <div className="flex items-start justify-between"><div><span className="text-xs font-black text-muted">УРОВЕНЬ {difficultyIndex + 1}</span><h2 className="mt-1 text-2xl font-black">{meta.title}</h2></div><span className="rounded-full bg-surface px-3 py-1.5 text-xs font-bold">{meta.count} упр.</span></div>
