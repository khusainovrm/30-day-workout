...
import { getFirstIncompleteExerciseIndex, isExerciseCompleted } from '../features/workout/transitions'
...
export function DayPage() {
  ...
  const sessionKey = `${program.id}:${dayNumber}`
  const matchingActiveSession = activeWorkoutSession?.programId === program.id && activeWorkoutSession.day === dayNumber
    ? activeWorkoutSession
    : null
  const savedCompletedExerciseIds = Array.from(new Set([
    ...(completedExercises[sessionKey] ?? []),
    ...(matchingActiveSession?.completedExerciseIds ?? [])
  ]))
  const hasSavedProgress = savedCompletedExerciseIds.length > 0
  const start = () => {
    if (matchingActiveSession) {
      navigate(`/workout/${program.id}/${dayNumber}`)
      return
    }
    const completedExerciseIds = isReplay ? [] : savedCompletedExerciseIds
    const exerciseIndex = isReplay ? 0 : getFirstIncompleteExerciseIndex(
      day.exercises.map(item => item.exerciseId),
      completedExerciseIds
    )
    setActiveSession({ programId: program.id, day: dayNumber, exerciseIndex, completedExerciseIds, state: 'exercise-preview', workoutStartedAt: Date.now(), totalPausedTime: 0 })
    navigate(`/workout/${program.id}/${dayNumber}`)
  }
  return <div className="min-h-[calc(100dvh-64px)] px-5 pt-3">
    ...
    <div className="grid gap-2">{day.exercises.map((item, index) => {
      const exercise = exercises[item.exerciseId]
      const completed = isReplay || isExerciseCompleted(savedCompletedExerciseIds, index, item.exerciseId)
      return <div key={`${item.exerciseId}-${index}`} className={`flex min-h-[70px] items-center gap-4 rounded-2xl px-3 py-2 ${completed ? 'bg-accent/15' : 'bg-card'}`}>
        <img src={exercise.images[0]} loading="lazy" alt="" className="size-14 rounded-xl object-cover" />
        <div className="min-w-0 flex-1"><p className="truncate font-extrabold">{exercise.name}</p><p className="text-sm font-semibold text-muted">{item.reps ? `${item.reps} повт.` : `${item.duration} сек.`}</p></div>
        {completed
          ? <span className="mr-1 flex items-center gap-1 text-xs font-extrabold text-ink"><CheckCircle2 size={18} className="text-green-700" />Готово</span>
          : <span className="mr-2 text-xs font-bold text-muted">{String(index + 1).padStart(2, '0')}</span>}
      </div>
    })}</div>
    <div className="sticky bottom-[86px] mt-6 rounded-[24px] bg-surface/90 pb-2 pt-3 backdrop-blur"><Button className="flex w-full items-center justify-center gap-2" onClick={start}><Play size={18} fill="currentColor" />{isReplay ? 'ПОВТОРИТЬ ТРЕНИРОВКУ' : matchingActiveSession || hasSavedProgress ? 'ПРОДОЛЖИТЬ ТРЕНИРОВКУ' : 'НАЧАТЬ ТРЕНИРОВКУ'}</Button></div>
  </div>
}
