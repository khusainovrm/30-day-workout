...
export function PwaPrompts() {
  ...
  return <div className="fixed inset-x-3 bottom-[calc(86px+env(safe-area-inset-bottom))] z-40 mx-auto max-w-[510px] rounded-[22px] border border-line bg-card p-4 shadow-2xl"><div className="flex gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-accent text-gray-950">{showUpdate ? <RefreshCcw size={20} /> : <Download size={20} />}</span><div><b>{showUpdate ? 'Update available' : 'Install app'}</b><p className="mt-0.5 text-sm text-muted">{showUpdate ? 'Get the newest improvements.' : 'Train offline and open it like a normal app.'}</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><Button variant="secondary" onClick={() => showUpdate ? setNeedRefresh(false) : dismissInstall()}>{showUpdate ? 'LATER' : 'NOT NOW'}</Button><Button onClick={() => showUpdate ? void updateServiceWorker(true) : void installEvent?.prompt()}>{showUpdate ? 'UPDATE' : 'INSTALL'}</Button></div></div>
}
